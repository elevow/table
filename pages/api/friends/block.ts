...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const rl = rateLimit(`POST /api/friends/block:${ip}`, { windowMs: 60 * 60 * 1000, max: 60 });
  if (!rl.allowed) return res.status(429).json({ error: 'Too many requests. Please slow down.' });

  try {
    const { userId, blockedId, reason } = (req.body || {}) as { userId?: string; blockedId?: string; reason?: string };
    ...
    const block = await service.block(userId, blockedId, reason);
