...
async function runCompleteRoomCodeMigration() {
  console.log('🔄 Complete Room Code Migration - Changes ALL room_id columns');

  console.log('📋 Please run these commands in your Supabase SQL editor:');
  console.log('\n-- ==== COMPLETE ROOM CODE MIGRATION (CHANGES ALL COLUMNS) ====');
...
WHERE (
  ...
  console.log('🚀 This changes ALL room_id columns to VARCHAR(8) to match game_rooms.id');
  console.log('💡 After running, room creation will work with codes like "WGDcNU4t"');
  console.log('');
  console.log('🔄 What this migration does:');
  console.log('   ✅ Changes game_rooms.id: UUID → VARCHAR(8)');
  console.log('   ✅ Changes active_games.room_id: UUID → VARCHAR(8)');
  console.log('   ✅ Changes chat_messages.room_id: UUID → VARCHAR(8)');
  console.log('   ✅ Changes friend_game_invites.room_id: UUID → VARCHAR(8)');
  console.log('   ✅ All foreign key types now match!');
}
