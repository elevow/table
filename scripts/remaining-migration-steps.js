...
async function generateRemainingMigrationSteps() {
  console.log('🔄 Remaining Migration Steps (8-10) - Handles existing policies');
  console.log('📋 Since you completed steps 1-7, here are the remaining steps:');
  console.log('\n-- ==== REMAINING MIGRATION STEPS (8-10) ====');
  
  console.log('\n-- STEP 8: Recreate RLS policies (Using CREATE OR REPLACE to handle existing policies)');
  
  console.log(`CREATE OR REPLACE POLICY spectators_can_view_public_games ON active_games
FOR SELECT USING (
  EXISTS (
  ...
  console.log(`CREATE OR REPLACE POLICY players_can_view_their_games ON active_games
FOR SELECT USING (
  EXISTS (
  ...
  console.log(`CREATE OR REPLACE POLICY room_creators_can_manage_games ON active_games
FOR ALL USING (
  EXISTS (
    SELECT 1 FROM game_rooms gr
    WHERE gr.id = active_games.room_id
  ...
  console.log(`CREATE OR REPLACE POLICY players_can_send_chat_messages ON chat_messages
FOR INSERT WITH CHECK (
  ...
  console.log(`CREATE OR REPLACE POLICY players_can_view_chat_messages ON chat_messages
FOR SELECT USING (
  EXISTS (
    SELECT 1 FROM active_games ag
    JOIN player_games pg ON pg.game_id = ag.id
  ...
  console.log(`CREATE OR REPLACE POLICY users_can_manage_own_invites ON friend_game_invites
FOR ALL USING (
  inviter_id = auth.uid() OR invitee_id = auth.uid()
);`);

  console.log(`CREATE OR REPLACE POLICY users_can_view_received_invites ON friend_game_invites
FOR SELECT USING (
  invitee_id = auth.uid()
);`);

  console.log(`CREATE OR REPLACE POLICY players_can_manage_own_rooms ON game_rooms
FOR ALL USING (
  created_by = auth.uid()
);`);

  console.log(`CREATE OR REPLACE POLICY spectators_can_view_public_rooms ON game_rooms
FOR SELECT USING (
  is_public = TRUE
);`);
...
WHERE (
  ...
  console.log('\n🎉 Remaining migration steps provided!');
  console.log('✅ Uses CREATE OR REPLACE to handle existing policies');
  console.log('📝 Copy and paste the commands above into your Supabase SQL editor');
  console.log('🚀 This should complete the migration without policy conflicts!');
  console.log('💡 After running, room creation will work with codes like "WGDcNU4t"');
}

generateRemainingMigrationSteps().catch(error => {
